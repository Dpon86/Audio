...
py manage.py system_check --fix >nul 2>&1
if %errorlevel% neq 0 (
    echo WARNING: System check found issues - continuing anyway
    echo Run 'py manage.py system_check --verbose' for details
)
...
start "Django Server - Port 8000" cmd /k "py manage.py runserver 8000"
