...
if (-not (Test-Path $venvPath)) {
    ...
    Write-Host "Please run scripts\setup\setup-venv.ps1 first to create the virtual environment" -ForegroundColor Yellow
...
$celeryScript = Join-Path $PSScriptRoot "start-celery.bat"
Start-Process -FilePath "cmd.exe" -ArgumentList "/k", "cd /d `"$backendRoot`" && scripts\startup\start-celery.bat" -WindowStyle Normal
